...
function generateDateRange(): string[] {
  ...
  const totalDays = (WEEKS_BACK + WEEKS_FORWARD + 1) * 7;
...
function getMonthLabel(date: string): string | null {
  ...
  // Show month label on the first Monday of the range
  if (d.getDay() === 1) {
...
interface WeekNavProps {
  weekDates: string[]; // still used for context but not for rendering
  selectedDate: string;
