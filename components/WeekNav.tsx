...
export default function WeekNav({ weekDates, selectedDate, onSelectDate, getTranscriptsForDate }: WeekNavProps) {
  return (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {transcripts.length > 0 ? (
                    <div className={`text-[10px] ${
                      selected && today ? "text-white/70" : selected ? "text-accent/70" : "text-muted"
                    }`}>
                      {transcripts.length} recording{transcripts.length !== 1 ? "s" : ""} · {formatDuration(totalDuration)}
                    </div>
                  ) : (
                    <div className={`text-[10px] ${
                      selected && today ? "text-white/50" : "text-gray-300"
                    }`}>
                      No recordings
                    </div>
                  )}
